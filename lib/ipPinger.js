...
import {PING_PREFIX} from './targetUtils.js';
...
export class IpPinger {
    constructor(settings, statusManager, logger) {
        ...
        this._targetCancellables = new Map();
        this._targetSubprocesses = new Map();
    }
    ...
    pingHost(target, onStatusChanged) {
        ...
        const existing = this._targetCancellables.get(target);
        if (existing) existing.cancel();
        const existingProc = this._targetSubprocesses.get(target);
        if (existingProc) existingProc.force_exit();

        const cancellable = new Gio.Cancellable();
        this._cancellables.add(cancellable);
        this._targetCancellables.set(target, cancellable);

        const forget = () => {
            this._cancellables.delete(cancellable);
            if (this._targetCancellables.get(target) === cancellable)
                this._targetCancellables.delete(target);
        };
        ...
        try {
            ...
            this._targetSubprocesses.set(target, proc);

            proc.wait_async(cancellable, (p, result) => {
                if (this._targetSubprocesses.get(target) === p)
                    this._targetSubprocesses.delete(target);
                forget();
                let reachable = false;
                try {
    ...
    destroy() {
        ...
        this._targetCancellables.clear();
        this._targetSubprocesses.forEach(p => p.force_exit());
        this._targetSubprocesses.clear();
    }
}
