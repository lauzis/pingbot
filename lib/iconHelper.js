...
// Icon filenames (single source of truth for both symbolic and material)
const ICON_FILENAMES = {
    'status-green': 'status-green.svg',
    'status-yellow': 'status-yellow.svg',
...
export class IconHelper {
     ...
     * @returns {Gio.BytesIcon|string} BytesIcon for symbolic/material, string for emoji
     */
    createPanelIcon(iconType) {
        const style = this.getIconStyle();
        
        if (style === 'symbolic') {
            return this._createFileIcon(iconType, 'symbolic');
    ...
    createPrefsIcon(iconType, Gtk) {
        ...
        if (style === 'symbolic') {
            const gicon = this._createFileIcon(iconType, 'symbolic');
            ...
            return new Gtk.Image({
                gicon: this._createFileIcon(iconType, 'material'),
                pixel_size: 16,
            });
        } else {
    ...
    updatePrefsIcon(widget, iconType) {
        ...
        if (widget.constructor.name === 'GtkImage') {
            if (style === 'symbolic') {
                ...
                widget.set_from_gicon(this._createFileIcon(iconType, 'material'));
            } else {
     ...
     * @param {string} iconType - One of IconType constants
     * @param {string} styleDir - 'symbolic' or 'material'
     * @returns {Gio.BytesIcon}
     * @private
     */
    _createFileIcon(iconType, styleDir) {
        ...
        try {
            // Load SVG content
            const [success, contents] = iconFile.load_contents(null);
            if (!success) {
                throw new Error(`Failed to load icon file: ${filename}`);
            }
            ...
            // This makes icons inherit the theme's foreground color
            svgContent = svgContent.replace(/fill="#[0-9a-fA-F]{6}"/g, 'fill="currentColor"');
            svgContent = svgContent.replace(/fill="#[0-9a-fA-F]{3}"/g, 'fill="currentColor"');
            // Also replace fill attributes without quotes (less common but possible)
            svgContent = svgContent.replace(/fill=#[0-9a-fA-F]{6}/g, 'fill=currentColor');
            svgContent = svgContent.replace(/fill=#[0-9a-fA-F]{3}/g, 'fill=currentColor');
            
            // Create BytesIcon from modified SVG
            const bytes = new TextEncoder().encode(svgContent);
            const gbytes = new GLib.Bytes(bytes);
            return new Gio.BytesIcon({ bytes: gbytes });
        } catch (e) {
            this._logger?.error(`Failed to load icon ${filename}`, e);
            // Fallback to FileIcon if modification fails
