...
import {isPingTarget, displayTarget} from './targetUtils.js';
...
export class PanelIndicator {
    constructor(uuid, name, settings, statusManager, onOpenPreferences, onForceRecheck, logger, extensionDir) {
        ...
        // handlers are tracked against `this` and released in destroy()
        this._settings.connectObject(
            'changed::icon-style', () => this._rebuildIndicator(),
            'changed::icon-size', () => this._rebuildIndicator(),
            this);
    }
    ...
    updateStatus(status) {
        ...
        const iconType = this._iconHelper.getIconTypeFromStatus(status);
        let cssClass;
    ...
    _addUrlMenuItem(url) {
        const isPing = isPingTarget(url);
        const label = displayTarget(url);
        const status = this._statusManager.getStatus(url);
        const iconType = this._iconHelper.getIconTypeFromStatus(status);
        const urlItem = this._iconHelper.createMenuItem(label, iconType, PopupMenu, St);
        ...
        if (!isPing) {
            ...
            urlItem.reactive = false;
            urlItem.can_focus = false;
        }
